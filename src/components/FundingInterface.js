...
const FundingInterface = ({ creditScore, creditData, onFundingComplete }) => {
  ...
  const { promise: toastPromise, error: toastError } = useToast();
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Connect your MetaMask wallet to access developer funding based on
            your credit score
          </p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div
              className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-${fundingTier.color}-100 text-${fundingTier.color}-800`}
            >
              {fundingTier.tier}
            </div>
          </div>
        ...
        <div className="mt-6 border-t border-gray-200 pt-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center">
              <svg
                className={`h-4 w-4 ${
                  creditScore >= 400 ? "text-green-500" : "text-red-500"
                }`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
              ...
              <span
                className={`ml-2 text-sm ${
                  creditScore >= 400 ? "text-green-700" : "text-red-700"
                }`}
              >
                Minimum credit score of 400 (
                {creditScore >= 400 ? "Met" : "Not Met"})
              </span>
            </div>
            <div className="flex items-center">
              <svg
      ...
      >
        <SuccessIllustration
          message={`${successDetails?.message} You have received ${formatUSDC(
