...
import IssueList from "@/components/github/IssueList/IssueList";
...
export default function Assignees() {
  ...
  const assigneeStats =
    filteredIssues?.reduce((stats, issue) => {
  ...
  const selectedIssues = filteredIssues?.filter((issue) => {
    ...
    return issue.assignees?.some(
      (assignee) => assignee.login === selectedAssignee
    );
  });
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead>
              <tr className="bg-gray-50">
                <th
                  scope="col"
                  className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-8"
                >
                  &nbsp;
                </th>
                <th
            ...
            <tbody className="divide-y divide-gray-200">
              ...
              >
                ...
                <td className="px-3 py-2 font-medium">Unassigned</td>
                <td className="px-3 py-2 text-right">
                  {assigneeStats.unassigned || 0}
                </td>
              </tr>
          ...
          title={
            selectedAssignee
              ? selectedAssignee === "unassigned"
                ...
                : `Issues Assigned to ${selectedAssignee} (${
