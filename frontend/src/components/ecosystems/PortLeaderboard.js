  ...
  if (!projects || projects.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
        <p className="text-gray-500">No projects found in this port yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
        <h3 className="text-lg font-semibold text-gray-900">Port Leaderboard</h3>
        <p className="text-sm text-gray-500">Top ships by shipping velocity</p>
      </div>
        ...
        {rankedProjects.map((project, index) => (
          <div key={project.id || project.slug} className="px-6 py-4 flex items-center hover:bg-gray-50 transition-colors">
            ...
            <div className="flex-shrink-0 text-right">
              <div className="text-sm font-bold text-gray-900">{project.velocity}</div>
              <div className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold">Velocity</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {projects.length > 10 && (
        <div className="px-6 py-3 bg-gray-50 text-center border-t border-gray-100">
          <Link href={`/ecosystems/${ecosystem}`} className="text-xs font-medium text-blue-600 hover:text-blue-800">
            View All Ships
          </Link>
        </div>
      )}
