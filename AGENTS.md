...
- `frontend/src/services/` — data + integrations; per-domain files (`ProjectDataService`, `DataServiceCore`, `walletService`, etc.). `DataService.ts` is a re-export shim — add new methods to `DataServiceCore`.
