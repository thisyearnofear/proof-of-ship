/**
 * This script imports projects directly from celoProjects.js to Firestore
 * 
 * To run:
 * 1. Set up your Firebase credentials in .env.local
 * 2. Run: node scripts/import-projects.js
 */

require('dotenv').config({ path: '.env.local' });
const admin = require('firebase-admin');
const path = require('path');

// Initialize Firebase Admin
const serviceAccount = {
  projectId: process.env.FIREBASE_PROJECT_ID,
  clientEmail: process.env.FIREBASE_CLIENT_EMAIL,
  privateKey: process.env.FIREBASE_PRIVATE_KEY?.replace(/\\n/g, '\n'),
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount)
  });
}

const db = admin.firestore();

async function importProjects() {
  try {
    // Import the celoProjects directly
    const { celoProjects } = require('../src/constants/celoProjects');
    
    console.log(`Found ${celoProjects.length} projects to import`);
    
    // Batch write to Firestore
    const batch = db.batch();
    
    for (const project of celoProjects) {
      // Generate a slug from the project name if not available
      const slug = project.slug || project.name.toLowerCase().replace(/\s+/g, '-');
      
      const projectRef = db.collection('projects').doc(slug);
      batch.set(projectRef, {
        name: project.name,
        slug: slug,
        description: project.description || '',
        contracts: project.contracts || [],
        socials: project.socials || {},
        founders: project.founders || [],
        season: project.season || null,
        owners: [], // Will be populated later
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      });
      
      console.log(`Added project ${project.name} to batch`);
    }
    
    // Commit the batch
    await batch.commit();
    console.log('Import completed successfully!');
    
  } catch (error) {
    console.error('Error during import:', error);
  }
}

// Run the import
importProjects().then(() => {
  console.log('Script execution completed');
  process.exit(0);
}).catch(err => {
  console.error('Script execution failed:', err);
  process.exit(1);
});
