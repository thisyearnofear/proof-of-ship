...
export const LiFiProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initializeLiFi = async () => {
      ...
      try {
        ...
        const chains = lifiChains.map(chain => {
          const networkConfig = networkConfigs[chain.id];
          
          return {
            id: chain.id,
            name: chain.name,
            token: chain.nativeCurrency?.symbol || (networkConfig?.nativeCurrency?.symbol || 'ETH'),
        ...
        for (const chain of chains) {
          try {
            ...
            const usdcToken = tokens[chain.id]?.find(token =>
              token.address.toLowerCase() === usdcAddresses[chain.id]?.toLowerCase()
            );
            
            // Store USDC and native token for each chain
            if (tokens[chain.id]) {
              const nativeToken = tokens[chain.id].find(token =>
        ...
        if (Object.keys(tokenData).length === 0) {
          chains.forEach(chain => {
            tokenData[chain.id] = [
              ...
              {
                address: usdcAddresses[chain.id] || '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48',
                symbol: 'USDC',
                name: 'USD Coin',
                decimals: 6,
                chainId: chain.id,
  ...
  const getChainLogoURI = (chainId) => {
    switch (chainId) {
      ...
      case 10: return 'https://raw.githubusercontent.com/ethereum-optimism/brand-kit/main/assets/svg/Optimism_Logo_Circle.svg';
      case 42161: return 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/arbitrum/info/logo.png';
      case 56: return 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/binance/info/logo.png';
      case 43114: return 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/avalanchec/info/logo.png';
      case 42220: return 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/celo/info/logo.png';
  ...
  const getTokenLogoURI = (symbol) => {
    switch (symbol) {
      case 'ETH': return 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2/logo.png';
      ...
      case 'AVAX': return 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/avalanchec/info/logo.png';
      case 'CELO': return 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/celo/info/logo.png';
      default: return 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/info/logo.png';
    }
  };

  // Get a quote for a cross-chain transfer
  const getQuote = async (fromChainId, toChainId, fromTokenAddress, toTokenAddress, fromAmount, options = {}) => {
  ...
  const getRoutes = async (fromChainId, toChainId, fromTokenAddress, toTokenAddress, fromAmount) => {
    if (!lifi) {
      throw new Error('LiFi not initialized');
    }
      ...
      for (let i = 0; i < updatedHistory.length; i++) {
        ...
        if (transfer.status !== 'DONE' && transfer.status !== 'FAILED') {
          try {
            ...
            console.error(`Failed to update status for transfer ${transfer.id}:`, err);
          }
        }
      }
