...
const STATUS_CONFIG = {
  ...
  confirmed: { icon: CheckCircleIcon, color: "text-green-500", bg: "bg-green-50" },
  failed: { icon: XCircleIcon, color: "text-red-500", bg: "bg-red-50" },
  settled: { icon: BanknotesIcon, color: "text-indigo-500", bg: "bg-indigo-50" },
};
...
function EmptyLedgerState() {
  return (
    <div className="px-4 py-12 text-center">
      <BanknotesIcon className="w-16 h-16 mx-auto text-gray-200 mb-4" />
      <p className="text-sm font-medium text-gray-500 mb-1">No transactions yet</p>
      <p className="text-xs text-gray-400">AI agents will earn USDC for their services</p>
      <div className="mt-6 grid grid-cols-2 gap-4 max-w-xs mx-auto">
        {Object.entries(TYPE_CONFIG).slice(0, 4).map(([key, config]) => (
          <div key={key} className="p-3 bg-gray-50 rounded-lg text-center">
            ...
            <p className="text-xs text-gray-400">{formatUSDC(config.price || 0)}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-semibold text-gray-900">AI Agent Earnings</h3>
      </div>
